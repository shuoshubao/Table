...
import { set, cloneDeep, isEqual, isUndefined, isFunction, isObject, merge, filter, find, flatten } from 'lodash';
            ...
            if (isFunction(render)) {
                if (isFunction(transform)) {
                    column.render = (text, record, index) => {
                        const value = transform(text, record, index);
...
// 编辑 数据处理
export const formatDataSource = ({ dataSource, index, dataIndex, value, columns }) => {
    const data = cloneDeep(dataSource);
    const { template } = find(columns, { dataIndex });
    const { tpl } = template;
