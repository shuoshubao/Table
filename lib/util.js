...
export const defaultExtraConfig = {
    ...
    showColumnsSetting: false, // 是否显示设置表头
    storageKey: '' // 存储的key
};

export const getStorageKey = storageKey => {
    return [componentName, 'ColumnsSetting', storageKey || window.location.pathname].join('__');
};
...
export const getTitleNode = (title, tooltip) => {
    if (!(tooltip || '').length) {
        return title;
    }
    return (
...
// 显示|隐藏
export const getRenderColumns = (columns, columnsTitleList) => {
    return columns
        .filter((v, i) => {
            // 不允许全都隐藏
        ...
        .map(v => {
            ...
            return {
                ...v,
                title: getTitleNode(title, tooltip)
            };
        });
};
