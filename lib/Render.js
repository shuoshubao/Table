...
import { Button, Typography, Tag, Tooltip, Popconfirm, Modal, Menu, Dropdown, Rate, Progress } from 'antd';
...
export default ({ dataIndex, transform, template }, context) => {
    ...
    return (text, record, index) => {
        ...
        // 行号
        if (tpl === 'sort') {
            const { handler = <MenuOutlined style={{ cursor: 'grab', color: '#999' }} /> } = template;
            return <DragHandle>{handler}</DragHandle>;
        }
        // 行号
        if (tpl === 'numbering') {
            return index + 1;
